...
class Vector2D:
    ...
    def __init__(self, param1: float = float(), param2: float = float()):
    ...
    def setMembersByRA(self, R: float, angle: float) -> None:
        self.setMembersByRAC(R, angle, 0, 0)
    ...
    def setMembersByRAC(self, R: float, angle: float, add_x: float, add_y: float) -> None:
        param2 = angle - math.pi / 2
        self.x = R * math.cos(param2) + add_x
        self.y = R * math.sin(param2) + add_y
